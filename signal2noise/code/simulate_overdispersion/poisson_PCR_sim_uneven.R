...
N_species <- 20 # if UNEVEN == TRUE, N_species must =20
UNEVEN <- TRUE # This is a flag for making the starting community unequal in abundance.

set.seed(151)
a_amp_vec <- c(1e6,
               1000,#999.999,1000.001,
               100,#99.999,100.001,
               50,#49.999,50.001,
               20,#19.999,20.001,
               10,#9.999,10.001,
               seq(4.9990,5.0010,by=0.0001))
...
# Assume all species are equally abundant (have an equivalent concentration)
lambda <- c(5,10,20,30,50,100,200,300,500,1000,3000,5000,10000,100000)
prop_true <- c(rep(0.2,2),rep(0.05,8),rep(0.02,10))
sum(prop_true)
...
for(k in 1:nrow(a_amp)){ # loop over amplification variability scenarios.
  ...
  for(i in 1:length(lambda)){
    W2[[i]] <- matrix(rpois(N_samp*N_species,lambda[i]*prop_true),N_samp,N_species,byrow=T)
  }
  ...
  for(j in 1:length(lambda)){
    ...
    for(i in 1:N_species){
      ...
      X[[j]][,i] <- rpois(N_samp,X[[j]][,i]*(1+0.9)^N_pcr2)
    }
  }
  ...
  for(i in 1:length(lambda)){
    ...
    W.summary$true.mean <- lambda[i] * prop_true
    ...
    Y.summary$true.mean <- lambda[i] * prop_true 
    ...
    Y.prob.summary$true.mean <- lambda[i] *prop_true
    Y.prob.summary$true.mean.prob <- prop_true
...
NOM <- paste0("./signal2noise/code/simulate_overdispersion/Skew simulation; N_species= ",N_species," pi = ",subsamp*100," N_pcr=",N_pcr1,".RData")
...
p_Y_zero <- ggplot(sim.all%>% filter(type=="Y")) +
  geom_line(aes(x=true.mean,y=p_zero,color=a,group=sp)) +
...
p_CV <- ggplot(sim.all%>% filter(type=="Y")) +
  geom_line(aes(x=true.mean,y=CV,color=a,group=sp)) +
  scale_x_continuous(expression("Mean("*lambda*")"),trans="log",breaks=BREAKS) +
...
p_var_prob_by_a_scen1   <- ggplot(sim.prob.dat %>% filter(
                                               #a_scenario >4,a_scenario<6,# c(5,10,50,100,1000,1e+06),
                                               lambda %in% c(1,10,100,1000,10000))) +
  geom_linerange(aes(ymin=q.0.05,ymax=q.0.95,x=lambda,group=sp,color=a),
                 position = position_dodge(width=1)) +
  scale_y_continuous("Proportion of Read") +
  scale_x_continuous(expression("Mean("*lambda*")"),trans="log",breaks=BREAKS) +
  scale_color_viridis_c("a",option="H") +
  facet_grid(true.mean.prob~a_scenario) +
  ggtitle("Range observed by a_scenario") +
  theme_bw()
...
p_SD_v_Mean <- ggplot(sim.prob.dat %>% filter( )  ) +
  geom_point(aes(x=mean.prob,y=sd.prob,color=lambda)) +
  scale_x_continuous(expression("Mean Proportion"),trans="log",breaks=BREAKS.PROBS,limits=c(0.001,0.5)) +
  scale_y_continuous("SD(Proportion)",trans="log",breaks=BREAKS.PROBS,limits=c(0.0001,NA)) +
  scale_color_viridis_c(expression(lambda),trans="log",option="H",breaks=BREAKS) +
  ggtitle("SD v Mean") +
  facet_wrap(~true.mean.prob) +
  theme_bw()

p_SD_v_Mean + facet_grid(true.mean.prob~a_scenario)

p_SD_v_Mean_low_a <- ggplot(sim.prob.dat %>% filter( a_scenario < 6)  ) +
  geom_point(aes(x=mean.prob,y=sd.prob,color=lambda)) +
  scale_x_continuous(expression("Mean Proportion"),trans="log",breaks=BREAKS.PROBS,limits=c(0.0001,0.5)) +
  scale_y_continuous("SD(Proportion)",trans="log",breaks=BREAKS.PROBS,limits=c(0.0001,NA)) +
  scale_color_viridis_c(expression(lambda),trans="log",option="H",breaks=BREAKS) +
  ggtitle("SD v Mean") +
  facet_wrap(~true.mean.prob) +
